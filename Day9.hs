...
main = withUtf8 $ do
    contents <- lines <$> readFile "input.txt" --  isGood ["11-09-01","09-11-01","31-09-11","16-05-18"] -- 
...
isGood :: [String] -> Bool
isGood inp = let x = map (\x -> read x :: Int) $ map (\x -> x !! 0) $ map (splitOn "-") inp
                 y = map (\x -> read x :: Int) $ map (\x -> x !! 1) $ map (splitOn "-") inp
                 z = map (\x -> read x :: Int) $ map (\x -> x !! 2) $ map (splitOn "-") inp
